  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <h1 className="font-headline text-xl font-bold text-slate-900">AI Assessment Content Studio</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-white p-6 rounded-2xl border border-slate-200 space-y-5 shadow-sm">
          <div>
            <h2 className="font-headline text-base font-bold text-slate-900">1. Upload Training Document</h2>
            <p className="text-xs text-slate-500 mt-0.5">Optional reference material for RAG context</p>
          </div>

          {/* File Upload Zone */}
          <div className="border border-dashed border-slate-300 hover:border-blue-500 rounded-2xl p-6 text-center space-y-2 bg-slate-50 transition cursor-pointer relative">
            <input
              type="file"
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-0.5 font-mono">PDF, PPT, DOCX (Max 25MB)</p>
            </div>
          </div>

          <div>
            <h2 className="font-headline text-base font-bold text-slate-900">2. Generation Settings</h2>
            <p className="text-xs text-slate-500 mt-0.5">Configure question parameters</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1.5">Competency Domain</label>
              <select
                value={competencyTag}
            ...
            <button
              ...
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl text-xs font-semibold shadow-sm transition flex items-center justify-center gap-2 mt-2"
            >
              {isGenerating ? (
                <>
        ...
        <div className="lg:col-span-7 bg-white p-6 rounded-2xl border border-slate-200 space-y-4 shadow-sm flex flex-col justify-between">
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div>
                <h2 className="font-headline text-base font-bold text-slate-900">Generated Questions</h2>
                <p className="text-xs text-slate-500 font-mono">{generatedQuestions.length} Questions Ready for Inspection</p>
              </div>
            ...
            {generatedQuestions.length > 0 ? (
              <div className="space-y-4">
                {generatedQuestions.map((q, idx) => (
                  <div key={q.id || idx} className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      <span className="text-xs font-bold text-blue-700 font-mono">Q0{idx + 1}</span>
                      <button
                    ...
                    <h4 className="text-xs sm:text-sm font-bold text-slate-900 leading-snug">{q.question_text}</h4>

                    <div className="space-y-1.5 text-xs text-slate-700">
                      <div className={`p-2 rounded-lg ${q.correct_option === 'option_a' ? 'bg-emerald-50 text-emerald-800 font-bold border border-emerald-300' : 'bg-white border border-slate-200'}`}>
                        A) {q.option_a}
                      </div>
                      <div className={`p-2 rounded-lg ${q.correct_option === 'option_b' ? 'bg-emerald-50 text-emerald-800 font-bold border border-emerald-300' : 'bg-white border border-slate-200'}`}>
                        B) {q.option_b}
                      </div>
                      {q.option_c && (
                        <div className={`p-2 rounded-lg ${q.correct_option === 'option_c' ? 'bg-emerald-50 text-emerald-800 font-bold border border-emerald-300' : 'bg-white border border-slate-200'}`}>
                          C) {q.option_c}
                        </div>
                      )}
                      {q.option_d && (
                        <div className={`p-2 rounded-lg ${q.correct_option === 'option_d' ? 'bg-emerald-50 text-emerald-800 font-bold border border-emerald-300' : 'bg-white border border-slate-200'}`}>
                          D) {q.option_d}
                        </div>
                      )}
                    </div>
              ...
              <div className="py-16 text-center text-slate-400 space-y-2">
                ...
                <p className="text-xs text-slate-500">Configure parameters on the left and click "Generate" to synthesize assessment questions with RAG.</p>
              </div>
            )}
          </div>
        </div>
