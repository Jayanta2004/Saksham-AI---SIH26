  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-headline text-xl font-bold text-slate-900">User &amp; Cadre Management</h1>
      ...
      {activeTab === 'active' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h2 className="font-headline text-base font-bold text-slate-900">Active Officers Directory</h2>
              <p className="text-xs text-slate-500 font-mono">{filteredActive.length} authorized officers in database</p>
            </div>
            ...
            <div className="relative w-full sm:w-80">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs sm:text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-500 transition"
              />
            </div>
          </div>
            ...
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-200 text-xs">
                {filteredActive.map((u) => {
                  ...
                  return (
                    <tr key={u.id || u.email} className="hover:bg-slate-50/60 transition">
                      <td className="py-3.5 px-3 font-bold text-slate-900 flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-xl bg-blue-50 text-blue-700 font-bold text-xs flex items-center justify-center shrink-0 border border-blue-200">
                          {(u.full_name || u.name || 'U').charAt(0)}
                        </div>
                        <span>{u.full_name || u.name}</span>
                      </td>
                      <td className="py-3.5 px-3 text-slate-500 font-mono">{u.email}</td>
                      <td className="py-3.5 px-3 text-slate-800 font-medium">{u.designation || 'Senior Statistical Officer'}</td>
                      <td className="py-3.5 px-3 text-slate-600">{u.department || 'National Accounts Division (NAD)'}</td>
                      <td className="py-3.5 px-3 text-slate-600">
                        <span className="bg-slate-100 text-slate-800 px-2 py-0.5 rounded font-mono text-[11px] font-semibold">
                          {u.cadre || 'ISS'}
                        </span>
                      </td>
      ...
      {activeTab === 'pending' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <div className="flex items-center space-x-2">
                <h2 className="font-headline text-base font-bold text-slate-900">Pending Verification Requests</h2>
                <span className="px-2 py-0.5 bg-amber-500/20 text-amber-800 border border-amber-500/30 text-xs font-mono font-bold rounded-full">
            ...
            <div className="relative w-full sm:w-80">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs sm:text-sm text-slate-900 placeholder:text-slate-400 outline-none"
              />
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs sm:text-sm border-collapse">
              <thead>
                <tr className="border-b border-slate-200 text-slate-700 text-xs font-semibold uppercase tracking-wider bg-amber-50/40">
                  <th className="py-3 px-3">Officer Name</th>
                  <th className="py-3 px-3">Official Email</th>
                  <th className="py-3 px-3">Designation</th>
                  <th className="py-3 px-3">Department</th>
              ...
              <tbody className="divide-y divide-slate-200 text-xs">
                {filteredPending.map((officer) => (
                  <tr key={officer.id} className="hover:bg-amber-50/30 transition">
                    <td className="py-3.5 px-3 font-bold text-slate-900 flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-xl bg-amber-100 text-amber-800 font-bold text-xs flex items-center justify-center shrink-0 border border-amber-300">
                        {officer.full_name.charAt(0)}
                      </div>
                      <span>{officer.full_name}</span>
                    </td>
                    <td className="py-3.5 px-3 text-slate-600 font-mono">{officer.email}</td>
                    <td className="py-3.5 px-3 text-slate-800 font-medium">{officer.designation}</td>
                    <td className="py-3.5 px-3 text-slate-600">{officer.department}</td>
                    <td className="py-3.5 px-3 text-slate-600">
                      <span className="bg-slate-100 text-slate-800 px-2 py-0.5 rounded font-mono text-[11px] font-semibold">
                        {officer.cadre}
                      </span>
                    </td>
                    <td className="py-3.5 px-3 text-slate-500 flex items-center gap-1 font-mono">
                    ...
                    <td className="py-3.5 px-3 text-right space-x-1.5">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 rounded-xl text-xs font-semibold inline-flex items-center gap-1 border border-red-200 transition"
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-5">
            <div className="flex items-center justify-between pb-4 border-b border-slate-200">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-xl bg-blue-50 text-blue-700 font-bold flex items-center justify-center border border-blue-200 text-base">
                  {(selectedUser.full_name || selectedUser.name || 'U').charAt(0)}
                </div>
                <div>
                  <h3 className="font-headline font-bold text-slate-900 text-base">{selectedUser.full_name || selectedUser.name}</h3>
                  <p className="text-xs text-slate-500 font-mono">{selectedUser.email}</p>
                </div>
              </div>
              <button onClick={() => setSelectedUser(null)} className="text-slate-400 hover:text-slate-600 p-1 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <span className="font-bold text-slate-900">{selectedUser.designation || 'Senior Statistical Officer'}</span>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <span className="text-slate-500 block mb-1">Department</span>
                <span className="font-bold text-slate-900">{selectedUser.department || 'National Accounts Division'}</span>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <span className="text-slate-500 block mb-1">Cadre Classification</span>
                <span className="font-bold text-slate-900 font-mono">{selectedUser.cadre || 'ISS'}</span>
              </div>
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <span className="text-slate-500 block mb-1">Overall Readiness</span>
