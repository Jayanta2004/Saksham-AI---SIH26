...
start "Saksham AI - Python Engine" cmd /k "cd /d %~dp0ai_service && python -m uvicorn main:app --host 127.0.0.1 --port 8000 --reload"

timeout /t 3 /nobreak >nul

echo [2/3] Starting Node.js API Gateway & RBAC Service (Port 5000)...
start "Saksham AI - API Gateway" cmd /k "cd /d %~dp0gateway_service && npm start"

timeout /t 3 /nobreak >nul

echo [3/3] Starting Next.js Web Frontend (Port 3000)...
start "Saksham AI - Next.js Frontend" cmd /k "cd /d %~dp0frontend && npm run dev"
