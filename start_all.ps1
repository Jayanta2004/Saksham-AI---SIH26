...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$baseDir\backend\ai_service'; python -m uvicorn main:app --host 127.0.0.1 --port 8000 --reload"

Start-Sleep -Seconds 3

Write-Host "[2/3] Starting Node.js API Gateway & RBAC Service (Port 5000)..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$baseDir\backend\gateway_service'; npm start"

Start-Sleep -Seconds 3

Write-Host "[3/3] Starting Vite React.js Web Frontend (Port 3000)..." -ForegroundColor Green
